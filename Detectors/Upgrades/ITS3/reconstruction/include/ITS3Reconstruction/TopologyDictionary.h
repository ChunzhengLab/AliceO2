...
class TopologyDictionary
{
 public:
  ...
  void reset() noexcept;
  void resetMaps(bool IB) noexcept;
  ...
  [[nodiscard]] float getErrX(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] float getZCOG(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] float getErrZ(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] float getErr2X(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] float getErr2Z(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] unsigned long getHash(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] int getNpixels(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] double getFrequency(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] bool isGroup(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] const itsmft::ClusterPattern& getPattern(int n, bool IB) const
  {
    const auto& data = (IB) ? mDataIB : mDataOB;
    assert(n >= 0 || n < (int)data.mVectorOfIDs.size());
  ...
  [[nodiscard]] TH1F* getTopologyDistribution(const std::string_view hname, bool IB) const;
