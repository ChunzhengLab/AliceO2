...
void alpideResponse(const std::string& inpath, const std::string& outpath, const std::string& chip_name)
{
    ...
    if (chip_name == "Alpide") {
        resp0.setColMax(o2::itsmft::SegmentationAlpide::PitchCol / 2.f);
        resp0.setRowMax(o2::itsmft::SegmentationAlpide::PitchRow / 2.f);
        resp1.setColMax(o2::itsmft::SegmentationAlpide::PitchCol / 2.f);
        resp1.setRowMax(o2::itsmft::SegmentationAlpide::PitchRow / 2.f);
        resp0.initData(0, inpath.c_str());
        resp1.initData(1, inpath.c_str());
    } else if (chip_name == "Apts") {
        resp1.setColMax(15.e-4 / 2.);
        resp1.setRowMax(15.e-4 / 2.);
        resp1.initData(1, inpath.c_str());
    } else if (chip_name == "Dummy") {
        resp1.setColMax(30.e-4 / 2.);
        resp1.setRowMax(30.e-4 / 2.);
        resp1.initData(1, inpath.c_str());
    } else {
    ...
    else if (chip_name == "Alpide") file->WriteObjectAny(&resp0, "o2::itsmft::AlpideSimResponse", "response0");
...
int main(int argc, const char* argv[])
{
    ...
    options.add_options()
        ...
        ("chip,c", bpo::value<std::string>()->default_value("Alpide"), "Chip name (Alpide or Apts).");
